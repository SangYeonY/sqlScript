...
CREATE TABLE table_date(
	acol DATE,
	bcol timestamp ,
	ccol timestamp DEFAULT sysdate	-- 값을 지정안했을 때 기본값(default) : sysdate 는 현재 날짜/시간
);
...
-- VARCHAR2(길이) : 가변형길이 단위 바이트, 길이는 최대길이 이고 실제로 메모리는 데이터크기만큼 사용합니다.
...
-- 여러가지 언어문자가 사용되고 저장될 때 NVARCHAR2 글자수로 계산하는 것이 권고사항입니다.
