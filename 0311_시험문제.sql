...
INSERT INTO tbl_member (mem_idx,name,email,tel,password)
	values(memidx_seq.nextval,'이하늬','honey@naver.com','010-0000-0000','1122');
insert into tbl_member(mem_idx,name,email,tel,password)
...
-- 문자열에서 날짜타입으는 자동캐스팅 됩니다. 함수를 사용할 때에는 to_date() 함수사용합니다.
...
-- '대여' 업무처리 (회원은 하루에 1개의 책만 대여할 수 있다.)
-- 1) 대여한다.	: insert 를 rent_date 만 하고 exp_date 는 rent_date + 14 로 한다.
...
-- 2)번에 대해서 프로시저 만들기
	SELECT rent_no, MEM_IDX FROM TBL_BOOKRENT tb 
		WHERE BCODE ='A1101' AND MEM_IDX = 10001 AND RETURN_DATE IS NULL;	-- 대여중인 책 중에서입니다.
...
END;	
SELECT * FROM TBL_BOOKRENT tb ;
-- 중요 : 변수선언할때 프로시저 생성하는 매개변수 또는 변수에서는 크기 생략하고varchar2, number 라고 선언
-- 일반 프로시저에서는 크기를 꼭 써줘야함. varchar2(20), number(3) 과 같이 선언한다.
DECLARE
	vresult varchar2;
BEGIN
